...
class UploadDataStep2VC: UIViewController {
    ...
    @IBAction private func uploadDataBtnTapped(_ sender: UIButton) {
        ...
        functions.httpsCallable("getUploadToken")
            .call(code) { [weak self] result, error in
                guard let self = self
                else {
                    Logger.DLog("self found to be nil")
                    return
                }
    ...
    func uploadFile(token: String, _ result: @escaping (Bool) -> Void) {
        ...
        managedContext.perform { [weak self] in
            ...
            let fileRef = Storage.storage(url: self?.storageUrl ?? "")
                .reference()
                .child("streetPassRecords/\(file)")
