...
class OTPViewController: UIViewController {
    ...
    @IBAction private func resendCode(_ sender: UIButton) {
    ...
    @IBAction private func verify(_ sender: UIButton) {
        verifyOTP { [weak self] status in
            switch status {
            case .InvalidOTP:
                self?.errorMessageLabel?.text = NSLocalizedString("InvalidOTP",
                                                                  comment: "Must be a 6-digit code")
                self?.errorMessageLabel?.isHidden = false
            case .WrongOTP:
                self?.errorMessageLabel?.text = NSLocalizedString("WrongOTP",
                                                                  comment: "Wrong OTP entered")
                self?.errorMessageLabel?.isHidden = false
            case .Success:
                if !UserDefaults.standard.bool(forKey: "hasConsented") {
                    self?.performSegue(withIdentifier: "showConsentFromOTPSegue",
                                       sender: self)
                } else if !UserDefaults.standard.bool(forKey: "allowedPermissions") {
                    self?.performSegue(withIdentifier: "showAllowPermissionsFromOTPSegue",
                                       sender: self)
                } else if !UserDefaults.standard.bool(forKey: "completedBluetoothOnboarding") {
                    self?.performSegue(withIdentifier: "OTPToTurnOnBtSegue",
                                       sender: self)
                } else {
                    self?.performSegue(withIdentifier: "OTPToHomeSegue",
                                       sender: self)
                }
            }
        }
